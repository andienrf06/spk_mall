...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Samasta Lifestyle Village (2) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Samasta Lifestyle Village (2)
        if ($row['namaGerai'] == 'Gerai Samasta Lifestyle Village (2)') {
            // Tandai bahwa data Gerai Samasta Lifestyle Village (2) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Samasta Lifestyle Village (2)
    ...
    // Jika data Gerai Samasta Lifestyle Village (2) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Samasta Lifestyle Village (2) tidak ditemukan.</p>";
    }
} else {
