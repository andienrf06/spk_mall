...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Discovery Shopping Mall (9) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Discovery Shopping Mall (9)
        if ($row['namaGerai'] == 'Gerai Discovery Shopping Mall (9)') {
            // Tandai bahwa data Gerai Discovery Shopping Mall (9) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Discovery Shopping Mall (9)
    ...
    // Jika data Gerai Discovery Shopping Mall (9) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Discovery Shopping Mall (9) tidak ditemukan.</p>";
    }
} else {
