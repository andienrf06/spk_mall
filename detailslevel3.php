...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Level 21 Mall (3) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Level 21 Mall (3)
        if ($row['namaGerai'] == 'Gerai Level 21 Mall (3)') {
            // Tandai bahwa data Gerai Level 21 Mall (3) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Level 21 Mall (3)
    ...
    // Jika data Gerai Level 21 Mall (3) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Level 21 Mall (3) tidak ditemukan.</p>";
    }
} else {
