...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Level 21 Mall (5) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Level 21 Mall (5)
        if ($row['namaGerai'] == 'Gerai Level 21 Mall (5)') {
            // Tandai bahwa data Gerai Level 21 Mall (5) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Level 21 Mall (5)
    ...
    // Jika data Gerai Level 21 Mall (5) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Level 21 Mall (5) tidak ditemukan.</p>";
    }
} else {
