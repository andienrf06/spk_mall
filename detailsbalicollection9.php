...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Bali Collection (9) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Bali Collection (9)
        if ($row['namaGerai'] == 'Gerai Bali Collection (9)') {
            // Tandai bahwa data Gerai Bali Collection (9) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Bali Collection (9)
    ...
    // Jika data Gerai Bali Collection (9) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Bali Collection (9) tidak ditemukan.</p>";
    }
} else {
