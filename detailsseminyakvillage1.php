...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Seminyak Village (1) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Seminyak Village (1)
        if ($row['namaGerai'] == 'Gerai Seminyak Village (1)') {
            // Tandai bahwa data Gerai Seminyak Village (1) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Seminyak Village (1)
    ...
    // Jika data Gerai Seminyak Village (1) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Seminyak Village (1) tidak ditemukan.</p>";
    }
} else {
