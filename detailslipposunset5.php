...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Bali Collection (5) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Bali Collection (5)
        if ($row['namaGerai'] == 'Gerai Lippo Plaza Sunset (5)') {
            // Tandai bahwa data Gerai Bali Collection (5) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Bali Collection (5)
    ...
    // Jika data Gerai Bali Collection (5) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Bali Collection (5) tidak ditemukan.</p>";
    }
} else {
