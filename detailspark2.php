...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Park 23 (2) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Park 23 (2)
        if ($row['namaGerai'] == 'Gerai Park 23 (2)') {
            // Tandai bahwa data Gerai Park 23 (2) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Park 23 (2)
    ...
    // Jika data Gerai Park 23 (2) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Park 23 (2) tidak ditemukan.</p>";
    }
} else {
