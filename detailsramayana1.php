...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Ramayana Bali Mall (1) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Ramayana Bali Mall (1)
        if ($row['namaGerai'] == 'Gerai Ramayana Bali Mall (1)') {
            // Tandai bahwa data Gerai Ramayana Bali Mall (1) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Ramayana Bali Mall (1)
    ...
    // Jika data Gerai Ramayana Bali Mall (1) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Ramayana Bali Mall (1) tidak ditemukan.</p>";
    }
} else {
