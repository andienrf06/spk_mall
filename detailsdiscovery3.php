...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Discovery Shopping Mall (3) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Discovery Shopping Mall (3)
        if ($row['namaGerai'] == 'Gerai Discovery Shopping Mall (3)') {
            // Tandai bahwa data Gerai Discovery Shopping Mall (3) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Discovery Shopping Mall (3)
    ...
    // Jika data Gerai Discovery Shopping Mall (3) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Discovery Shopping Mall (3) tidak ditemukan.</p>";
    }
} else {
