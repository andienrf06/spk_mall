...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Lippo Plaza Renon (2) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Lippo Plaza Renon (2)
        if ($row['namaGerai'] == 'Gerai Lippo Plaza Renon (2)') {
            // Tandai bahwa data Gerai Lippo Plaza Renon (2) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Lippo Plaza Renon (2)
    ...
    // Jika data Gerai Lippo Plaza Renon (2) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Lippo Plaza Renon (2) tidak ditemukan.</p>";
    }
} else {
