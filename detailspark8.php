...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Park 23 (8) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Park 23 (8)
        if ($row['namaGerai'] == 'Gerai Park 23 (8)') {
            // Tandai bahwa data Gerai Park 23 (8) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Park 23 (8)
    ...
    // Jika data Gerai Park 23 (8) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Park 23 (8) tidak ditemukan.</p>";
    }
} else {
