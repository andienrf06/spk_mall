...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Discovery Shopping Mall (5) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Discovery Shopping Mall (5)
        if ($row['namaGerai'] == 'Gerai Discovery Shopping Mall (5)') {
            // Tandai bahwa data Gerai Discovery Shopping Mall (5) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Discovery Shopping Mall (5)
    ...
    // Jika data Gerai Discovery Shopping Mall (5) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Discovery Shopping Mall (5) tidak ditemukan.</p>";
    }
} else {
