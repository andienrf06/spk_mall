...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Living World Denpasar (1) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Living World Denpasar (1)
        if ($row['namaGerai'] == 'Gerai Living World Denpasar (1)') {
            // Tandai bahwa data Gerai Living World Denpasar (1) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Living World Denpasar (1)
    ...
    // Jika data Gerai Living World Denpasar (1) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Living World Denpasar (1) tidak ditemukan.</p>";
    }
} else {
