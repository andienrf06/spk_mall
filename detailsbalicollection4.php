...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Bali Collection (4) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Bali Collection (4)
        if ($row['namaGerai'] == 'Gerai Bali Collection (4)') {
            // Tandai bahwa data Gerai Bali Collection (4) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Bali Collection (4)
    ...
    // Jika data Gerai Bali Collection (4) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Bali Collection (4) tidak ditemukan.</p>";
    }
} else {
