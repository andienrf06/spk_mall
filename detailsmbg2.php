...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Mall Bali Galeria (2) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Mall Bali Galeria (2)
        if ($row['namaGerai'] == 'Gerai Mall Bali Galeria (2)') {
            // Tandai bahwa data Gerai Mall Bali Galeria (2) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Mall Bali Galeria (2)
    ...
    // Jika data Gerai Mall Bali Galeria (2) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Mall Bali Galeria (2) tidak ditemukan.</p>";
    }
} else {
