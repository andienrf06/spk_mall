...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Sidewalk Jimbaran (2) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Sidewalk Jimbaran (2)
        if ($row['namaGerai'] == 'Gerai Sidewalk Jimbaran (2)') {
            // Tandai bahwa data Gerai Sidewalk Jimbaran (2) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Sidewalk Jimbaran (2)
    ...
    // Jika data Gerai Sidewalk Jimbaran (2) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Sidewalk Jimbaran (2) tidak ditemukan.</p>";
    }
} else {
