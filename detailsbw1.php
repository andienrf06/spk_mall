...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Beachwalk Shopping Centre (1) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Beachwalk Shopping Centre (1)
        if ($row['namaGerai'] == 'Gerai Beachwalk Shopping Centre (1)') {
            // Tandai bahwa data Gerai Beachwalk Shopping Centre (1) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Beachwalk Shopping Centre (1)
    ...
    // Jika data Gerai Beachwalk Shopping Centre (1) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Beachwalk Shopping Centre (1) tidak ditemukan.</p>";
    }
} else {
