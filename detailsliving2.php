...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Living World Denpasar (2) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Living World Denpasar (2)
        if ($row['namaGerai'] == 'Gerai Living World Denpasar (2)') {
            // Tandai bahwa data Gerai Living World Denpasar (2) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Living World Denpasar (2)
    ...
    // Jika data Gerai Living World Denpasar (2) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Living World Denpasar (2) tidak ditemukan.</p>";
    }
} else {
