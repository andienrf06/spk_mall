...
// Check if data is retrieved successfully
if ($data) {
    $geraiBaliCollectionFound = false; // Inisialisasi flag untuk menandai apakah data Gerai Lippo Plaza Sunset (6) ditemukan
    foreach ($data as $row) {
        // Check apakah nama gerai adalah Gerai Lippo Plaza Sunset (6)
        if ($row['namaGerai'] == 'Gerai Lippo Plaza Sunset (6)') {
            // Tandai bahwa data Gerai Lippo Plaza Sunset (6) ditemukan
            $geraiBaliCollectionFound = true;

            // Tampilkan detail data untuk Gerai Lippo Plaza Sunset (6)
    ...
    // Jika data Gerai Lippo Plaza Sunset (6) tidak ditemukan, tampilkan pesan
    if (!$geraiBaliCollectionFound) {
        echo "<p>Data untuk Gerai Lippo Plaza Sunset (6) tidak ditemukan.</p>";
    }
} else {
